      ...
      <main className="pt-14 max-w-4xl mx-auto px-4 py-10">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Knowledge Timeline</h1>
          </div>
          ...
          <div className="border border-slate-100 rounded-2xl p-5 bg-white">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-800">Hidden Patterns</span>
              </div>
              <button
                onClick={loadPatterns}
                disabled={patternsLoading}
                className="flex items-center gap-1.5 px-3 py-1 rounded-lg text-[11px] bg-slate-900 text-white hover:bg-slate-850 transition-all disabled:opacity-50 font-semibold"
              >
                {patternsLoading
            ...
            {patterns.map((p, i) => (
              ...
              >
                <Lightbulb className="w-3.5 h-3.5 text-slate-700 flex-shrink-0 mt-0.5" />
                <p className="text-xs text-slate-650 leading-relaxed">{p}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-sm text-slate-450">No content saved yet.</p>
            <Link href="/capture" className="text-xs text-slate-900 hover:underline mt-1 inline-block font-semibold">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="absolute left-0 top-1 w-10 h-10 flex items-center justify-center">
                      <div className="w-2.5 h-2.5 rounded-full bg-slate-800 ring-4 ring-slate-100 ring-offset-2 ring-offset-white" />
                    </div>

                    {/* Month header */}
                    <div className="mb-3">
                      <h2 className="text-sm font-bold text-slate-850">{label}</h2>
                      <p className="text-[11px] text-slate-400 mt-0.5">
                        {items.length} item{items.length !== 1 ? 's' : ''} saved
                      </p>
                    </div>

                    <div className="border border-slate-100 rounded-2xl p-5 space-y-4 bg-white">
                      {/* Stats */}
                      <div className="flex items-center gap-3 flex-wrap">
                        {Object.entries(byType).map(([type, count]) => {
                          const Icon = typeIcons[type] || FileText
                          return (
                            <div key={type} className="flex items-center gap-1.5 text-[11px] text-slate-450 font-semibold">
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {topTags.map(([tag, count]) => (
                              <Badge
                                key={tag}
                                variant="secondary"
                                className="text-[10px] bg-slate-50 text-slate-600 border border-slate-200 hover:bg-slate-100"
                              >
                                {tag}
                      ...
                      <div className="space-y-1.5">
                        {items.slice(0, 5).map(item => (
                          <div key={item.id} className="flex items-center gap-2 py-1">
                            ...
                            <span className="text-xs text-slate-650 truncate">{item.title}</span>
                            {item.source_name && (
